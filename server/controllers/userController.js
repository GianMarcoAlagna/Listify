...
const userController = {
  createUser: async function (req, res, next) {
    ...
    try {
      ...
      return next(
        createError(err, 500, "Error in find", "userController", "createUser")
      );
    }
    if (user) {
      return next(
  ...
  updateUserData: async function (req, res, next) {
    ...
    try {
      if (oAuthUser === "true") {
        ...
        try {
          await OAuthUser.findOneAndUpdate(
            ...
            { todo: todo, textEditor: textEditor }
          );
        } catch (err) {
          return next(
            createError(
      ...
      return next(
        createError(
          (err, 404, "User Is Invalid", "userController", "updateUserData")
        )
      );
    }
    return next();
  ...
  getUser: async function (req, res, next) {
    ...
    try {
      ...
      return next(
        createError(err, 404, "Couldn't Find User", "userController", "getUser")
      );
    }
  },
    ...
    try {
      ...
      return next(
        createError(
          err,
          404,
          "Error in find, couldn't find user",
          "userController",
          "authenticateUser"
        )
      );
